...
END_SHADER
  ...
  def activate_current_location
    objects = @world.objects_for_world_coord(@player.x,@player.y)
    obj = objects.first
    if obj.is_a? CaveEntrance
      @current_cave = obj
    end
  end
  ...
  def generate_new_chunks(chunk_x, chunk_y)
    view_dist = 1
    (chunk_x-view_dist..chunk_x+view_dist).each do |cx|
      (chunk_y-view_dist..chunk_y+view_dist).each do |cy|
          ...
          if @world.has_chunk? chunk_x, chunk_y
            ...
            @world.chunk_size.times do |px|
              @world.chunk_size.times do |py|
                ...
                if $debug && (px % @world.chunk_size == 0 ||
                  py % @world.chunk_size == 0)
                    i = 0
                    c = Color.rgba(250,0,0,150)
                    size = 31-i
                    draw_quad(x+i+1, y+i+1, c, 
                              x+size, y+i+1, c, 
                              x+size, y+size, c, 
  ...
  def button_down(id)
    ...
    if id == KbLeft
      ...
      @light_sources = []
    elsif id == KbEnter || id == KbReturn
      activate_current_location
